  ...
  return (
    <section id="stats" className="section" style={{
      ...
      <div className="container" ref={ref}>
        <div>
          <RevealText className="section-title">Proven Impact</RevealText>
          <motion.p
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
          {stats.map((stat, index) => (
            ...
            >
              ...
              <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: 'var(--gradient-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem', color: 'var(--color-primary)' }}>
                <stat.icon size={28} />
              </div>
              <div style={{ fontSize: '3rem', fontWeight: 700, fontFamily: 'var(--font-serif)', background: 'var(--gradient-primary)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '0.5rem', lineHeight: 1 }}>
                <AnimatedCounter value={stat.value} prefix={stat.prefix} suffix={stat.suffix} isInView={isInView} />
              </div>
              <h3 style={{ fontSize: '1.125rem', fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.5rem', fontFamily: 'var(--font-sans)' }}>{stat.label}</h3>
              <p style={{ fontSize: '0.875rem', color: 'var(--color-text-light)' }}>{stat.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
